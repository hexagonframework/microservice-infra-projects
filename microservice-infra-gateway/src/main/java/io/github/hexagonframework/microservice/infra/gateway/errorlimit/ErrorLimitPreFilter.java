 ...
 * @author Xuegui Yuan
 */
public class ErrorLimitPreFilter extends AbstractErrorLimitFilter {

    private final ErrorLimiter limiter;

    public ErrorLimitPreFilter(ErrorLimiter limiter, ErrorLimitProperties properties, RouteLocator routeLocator) {
    ...
    public Object run() {
        ...
        Optional.ofNullable(policy()).ifPresent(policyKeyValue -> {
                ...
                final ErrorRate errorRate = this.limiter.query(policyKeyValue.getErrorLimitPolicy(), key(request, policyKeyValue.getKey(), policyKeyValue.getErrorLimitPolicy().getType()));
                if (errorRate != null) {
